		...
		for (int i = 0; i < hits.Num(); i++) {
			...
			if (actorHit && hits[i].GetActor()) {
				...
				if (AEnemyCharacter* enemy = Cast<AEnemyCharacter>(hits[i].GetActor())) {
					if (enemy->TakeDamage(hits[i].Distance, Damage - FMath::RandRange(0, ((int)(OwnerCharacter->DamageBuff / 2) + 3)) + OwnerCharacter->DamageBuff, GetActorLocation(), KnockbackForce / enemiesHit, true)) {
						KilledEnemy();
					}
					enemiesHit++;
				}
